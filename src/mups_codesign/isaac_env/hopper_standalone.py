...
from legged_gym.utils.math import quat_apply_yaw, wrap_to_pi, torch_rand_sqrt_float, quat_to_euler
from legged_gym.utils.helpers import class_to_dict

class HopperStandalone(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        # Init privileged mass params early so create_envs() can access it
    ...
    def set_design_params(self, params=None):
        ...
        # sweep ks and l0 to get the "optimal" policy for each design
        # params_bounds = np.array([
        #     [ks * 0.2, ks * 2.2],
        #     [l0 * 0.7, l0 * 1.2],
        # ])
        # num_grid = np.sqrt(self.num_envs).astype(int)
        # assert (num_grid**2 == self.num_envs), "num_envs must be a perfect square for grid search"

        # ks_range = np.linspace(params_bounds[0, 0], params_bounds[0, 1], num=num_grid)
        # l0_range = np.linspace(params_bounds[1, 0], params_bounds[1, 1], num=num_grid)
        # ks_grid, l0_grid = np.meshgrid(ks_range, l0_range)

        # params = np.stack([ks_grid.flatten(),l0_grid.flatten(),], axis=-1) # (num_envs, 2)

        # self.design_params[:, 0:2] = torch.from_numpy(params).to(self.device)
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_center_of_mass:
            ...
            rand_com = np.array([
                np.random.uniform(com_ranges[0], com_ranges[1]),  # x
                0.0,                                              # y stays fixed
                np.random.uniform(com_ranges[0], com_ranges[1])   # z
            ])
    ...
    def _init_buffers(self):
        ...
        # kp_kd_multipliers[0] is multiplier for Kp, kp_kd_multipliers[1] is multiplier for Kd
        self.kp_kd_multipliers = (self.cfg.domain_rand.kp_kd_range[1] \
                               ...
                               * torch.rand(2, self.num_envs, self.num_actions, dtype=torch.float, 
                               ...
                               + self.cfg.domain_rand.kp_kd_range[0]
        if not self.cfg.domain_rand.randomize_kp_kd:
            self.kp_kd_multipliers = torch.ones_like(self.kp_kd_multipliers)
    ...
    def draw_debug_vis_srb(self, srb_state):
        if not self.viewer:
            return
        ...
        box_geom = gymutil.WireframeBoxGeometry(xdim=0.6, ydim=0.15, zdim=0.15, pose=None, color=(0, 1, 0))

        for idx in range(self.num_envs):
            pos = srb_state[idx, :3].cpu().numpy()
            orn = srb_state[idx, 3:7].cpu().numpy()

            box_pose = gymapi.Transform()
            box_pose.p = gymapi.Vec3(pos[0], pos[1], pos[2])
            box_pose.r = gymapi.Quat(orn[0], orn[1], orn[2], orn[3])

            gymutil.draw_lines(box_geom, self.gym, self.viewer, self.envs[idx], box_pose)
            box_pose.p += gymapi.Vec3(0.001, 0.0, 0.0)
            gymutil.draw_lines(box_geom, self.gym, self.viewer, self.envs[idx], box_pose)
            box_pose.p += gymapi.Vec3(-0.001, 0.0, 0.0)
            gymutil.draw_lines(box_geom, self.gym, self.viewer, self.envs[idx], box_pose)
            box_pose.p += gymapi.Vec3(0.0, 0.001, 0.0)
            gymutil.draw_lines(box_geom, self.gym, self.viewer, self.envs[idx], box_pose)
            box_pose.p += gymapi.Vec3(0.0, -0.001, 0.0)
            gymutil.draw_lines(box_geom, self.gym, self.viewer, self.envs[idx], box_pose)
