...
class ActorCritic(nn.Module):
    ...
    def __init__(self,  num_proprio,                        # NEW (renamed)       
        ...
        # [cur. obs | latent | scan (optional) | estimated_obs ]
        if self.num_scan_obs == 0:
            mlp_input_dim_a = num_proprio + latent_encoder_output_dim + num_estimated_obs
        else:
            mlp_input_dim_a = num_proprio + latent_encoder_output_dim + scan_encoder_output_dim + num_estimated_obs
    ...
    def update_distribution(self, obs_buf, privileged_obs_buf, estimated_obs_buf, scan_obs_buf, adaptation_mode=False):
        ...
        # Extract current observation (last num_proprio features from flattened buffer)
        cur_obs_only = obs_buf[:, -self.num_proprio:]
        if self.num_scan_obs == 0 or scan_obs_buf is None:
            actor_input = torch.cat((cur_obs_only, priv_latent, estimated_obs_buf), dim=-1)
        else:
            scan_latent = self.scan_encoder(scan_obs_buf)
            actor_input = torch.cat((cur_obs_only, priv_latent, scan_latent, estimated_obs_buf), dim=-1)
