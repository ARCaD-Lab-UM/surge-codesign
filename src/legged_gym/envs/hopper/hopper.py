...
class HopperRobot(LeggedRobot):
    ...
    def set_design_params(self, param_dict={}):
        """Set the UPS spring design parameters for every environment.

        Args:
            param_dict (dict): optional mapping of design parameter name
                ("ups_ks", "ups_l0", "ups_l2", "ups_l4") to value(s) of shape
                (num_envs,) or (1,). Missing keys fall back to nominal values.

        During training (cfg.domain_rand.randomize_design=True) the swept parameters
        ks, l0, l2, l4 are randomized per environment within their design bounds.
        Codesign evaluation disables this and sets the parameters explicitly instead.
        """
        ...
        # During training, randomize the swept parameters (ks, l0, l2, l4) per env so
        # the policy generalizes across designs. Disabled for codesign evaluation.
        if getattr(self.cfg.domain_rand, "randomize_design", False):
