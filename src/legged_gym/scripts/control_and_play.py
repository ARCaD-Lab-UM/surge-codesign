...
def play(args):
    ...
    env_cfg.commands.heading_command = False   # Turn off automatic heading calculation

    # Initialize gamepad
    gamepad = Gamepad(0.25, 0.0, 0.0)           # Manually have to calibrate with rc_scale :(
    ...
    for i in range(10*int(env.max_episode_length)):
        ...
        # # Get estimated obs
        # with torch.no_grad():
        #     estimator_output = ppo_runner.alg.estimator(obs)

        estimator_output = estimated_obs

        # Compute actions & step
        actions = inference_policy(obs.detach(), privileged_obs.detach(), estimator_output.detach(), scan_obs.detach(), adaptation_mode=True) # use adaption module
        obs, privileged_obs, critic_obs, estimated_obs, scan_obs, rews, dones, infos = env.step(actions.detach())
        time.sleep(max(env.dt - (time.perf_counter() - time_start), 0))
        if RECORD_FRAMES:
            if i % 2:
